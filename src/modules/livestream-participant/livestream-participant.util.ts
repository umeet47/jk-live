import { LiveStreamParticipant } from "@prisma/client";

export const filterLiveStreamParticipantData = async (
    streams: LiveStreamParticipant[],
) => {
    ...
    const dailyDurations = filteredStreams.reduce((result, stream) => {
        ...
        if (!result[dayKey]) {
            result[dayKey] = { date: dayKey, totalDuration: 0, role: stream.role, userId: stream.userId };
        }

        result[dayKey].totalDuration += durationInSeconds;
        return result;
    }, {} as Record<string, { date: string; totalDuration: number; role: string; userId: string }>);

    // Convert the result object into an array and format totalDuration as "hh:mm:ss"
    return Object.values(dailyDurations).map(({ date, totalDuration, role, userId }) => {
        ...
        return { date, totalTimeDuration, role, userId };
    });
};